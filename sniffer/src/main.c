...
#define UART_RX 	P02
	...
	UINT8 u8StateRxLFP = STATE_RxLFP_INIT; // State Rx LineFi Packet
		...
		switch(u8StateRxLFP) {
			case STATE_RxLFP_INIT :
				if (Receive_Data_From_UART1_nb(&u8RxUART)) {
					...
					u8StateRxLFP = STATE_RxLFP_START;
				}
				break;
			...
			case STATE_RxLFP_END :
				...
				switch(cp_buf2linefipacket(u8RxIdx, pu8RxUART, &stLineFiPkt)) {
					...
					case CONV_ERR_CRC :
						...
						break;
					case CONV_ERR_TOO_SMALLSIZE :
						printf_fast_f("ERROR: too small length!!\r\n");
						break;
				}
				u8StateRxLFP = STATE_RxLFP_INIT;
				break;
		} //switch(u8StateRxLFP)
	} //while(1)
}
